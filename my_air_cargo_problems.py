...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        def unload_actions():
            ...
            # precond: In(cargo,plane) ^ At(plane,airport)
            # effect: At(cargo,airport) ^ -In(cargo,plane)
...
def air_cargo_p2() -> AirCargoProblem:
    ...
    neg = [ expr('At(C1, JFK)'),    # C1 not at JFK or ATL
            expr('At(C1, ATL)'),
            expr('In(C1, P1)'),     # C1 not in a plane
            expr('In(C1, P2)'),
            expr('In(C1, P3)'),

            expr('At(C2, SFO)'),    # C2 not at SFO or ATL
            expr('At(C2, ATL)'),
            expr('In(C2, P1)'),     # C2 not in a plane
            expr('In(C2, P2)'),
            expr('In(C2, P3)'),

            expr('At(C3, JFK)'),    # C3 not at JFK or SFO
            expr('At(C3, SFO)'),
            expr('In(C3, P1)'),     # C3 not in a plane
            expr('In(C3, P2)'),
            expr('In(C3, P3)'),

            expr('At(P1, JFK)'),    # P1 not at JFK or ATL
            expr('At(P1, ATL)'),

            expr('At(P2, SFO)'),    # P2 not at SFO or ATL
            expr('At(P2, ATL)'),

            expr('At(P2, JFK)'),    # P3 not at SFO or JFK
            expr('At(P2, SFO)'),
          ]
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    neg = [ expr('At(C1, JFK)'),    # C1 not at JFK, ATL, ORD
            expr('At(C1, ATL)'),
            expr('At(C1, ORD)'),
            expr('In(C1, P1)'),     # C1 not in a plane
            expr('In(C1, P2)'),

            expr('At(C2, SFO)'),    # C2 not at SFO, ATL, ORD
            expr('At(C2, ATL)'),
            expr('At(C2, ORD)'),
            expr('In(C2, P1)'),     # C2 not in a plane
            expr('In(C2, P2)'),

            expr('At(C3, JFK)'),    # C3 not at JFK, SFO, ORD
            expr('At(C3, SFO)'),
            expr('At(C3, ORD)'),
            expr('In(C3, P1)'),     # C3 not in a plane
            expr('In(C3, P2)'),
            ...
            expr('At(P2, SFO)'),    # P2 not at SFO, ATL, ORD
            expr('At(P2, ATL)'),
            expr('At(P2, ORD)'),
          ]
